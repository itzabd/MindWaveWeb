...
localai_client = localai.Client(base_url=os.getenv("LOCALAI_URL"))
eeg_processor = EEGProcessor(localai_client)


# Add this function to create tables if they don't exist
def initialize_supabase_tables():
    try:
        # Create EEG results table
        supabase.rpc('create_table_if_not_exists', {
            'table_name': 'eeg_results',
            'table_definition': '''
    ...
    if request.method == 'POST':
        ...
        if file:
            try:
                ...
                filepath = os.path.join('eeg_data', unique_filename)

                # Ensure directory exists
                os.makedirs('eeg_data', exist_ok=True)
                file.save(filepath)

                # Process and classify the EEG data
                eeg_data = eeg_processor.load_gdf(filepath)
                features = eeg_processor.preprocess(eeg_data)
                result = eeg_processor.classify(features)

                # Save results to Supabase
                response = supabase.table('eeg_results').insert({
                ...
                if response.data:
                    flash('EEG data processed successfully!', 'success')
                    return render_template('eeg_result.html',
                                           result=result,
                                           filename=filename)
                else:
                    flash('Failed to save results to database', 'danger')
                    return redirect(request.url)

            except Exception as e:
                flash(f'Error processing EEG data: {str(e)}', 'danger')
                return redirect(request.url)

    return render_template('upload_eeg.html')


@app.route('/eeg/results')
@login_required
def view_eeg_results():
    # Fetch user's EEG results
    response = supabase.table('eeg_results').select('*').eq('user_id', current_user.id).execute()
    results = response.data if response.data else []

    # Format timestamps
    for result in results:
        if 'timestamp' in result:
            result['timestamp'] = datetime.fromisoformat(result['timestamp']).strftime('%Y-%m-%d %H:%M:%S')

    return render_template('eeg_results.html', results=results)


@app.route('/eeg/result/<result_id>')
@login_required
def view_eeg_result(result_id):
    # Fetch specific EEG result
    response = supabase.table('eeg_results').select('*').eq('id', result_id).eq('user_id', current_user.id).execute()

    if not response.data:
        flash('Result not found', 'danger')
        return redirect(url_for('view_eeg_results'))
    ...
    return render_template('eeg_result_detail.html', result=result)
