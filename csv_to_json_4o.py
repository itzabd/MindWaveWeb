...
def csv_to_json(df, csp, window_size, selected_columns, labels):
    ...
    :param csp: CSP features (can be None)
    ...
    """
    json_array = []
    channel_names = ['FCz', 'C3', 'Cz', 'C4', 'CP3']  # Channel name mapping

    for start in range(0, len(df) - window_size + 1, window_size):
        window_data = df.iloc[start:start + window_size, :]
        label = str(int(float(labels[start])))  # More robust label conversion

        # Extract frequency features
        features = extract_features(window_data, selected_columns)

        # Handle CSP features (if provided)
        csp_features = ""
        if csp is not None:
            try:
                cspdata = pd.DataFrame(csp[int(start / window_size)]).T
                features = pd.concat([features, cspdata], axis=1)
                csp_features = f"\nCSP values: 0: {cspdata.values[0][0]:.4f}, 1: {cspdata.values[0][1]:.4f}"
            except (TypeError, IndexError) as e:
                print(f"Warning: CSP processing failed at window {start}: {str(e)}")
                csp_features = "\nCSP values: Not available"

        features_dict = features.to_dict('index')[0]
        ...
        for i, (channel_idx, bands) in enumerate(selected_columns):
            ...
            for band in bands if isinstance(bands, list) else [bands]:
                ...
                power_value = features_dict.get(band_key, 0)  # Safer dictionary access

                if isinstance(power_value, (np.ndarray, list)):
                    power_value = power_value.item() if power_value.size == 1 else power_value[0]
                features_list.append(f"Power in {band[0]}-{band[1]} Hz: {power_value:.4f}")
            features_dict_with_keys[key] = features_list

        # Create prompt
        system_message = {
            "role": "system",
            "content": "Analyze EEG features and predict the label (integer only)."
        }
